...
`access clear-retry` 可用 `--main-password`、`--management-password` 或 `--two-factor` 选择清理范围；2FA 只清理每日限额，不解除固定 10 秒动态码退避。`access clear-sessions` 清除全部 DSH 和管理中心会话，添加 `--management-only` 时只清除管理中心会话。关闭 2FA 或管理中心密码不注销现有会话；重设主密码清除全部会话，设置或重设管理中心密码清除全部管理中心会话。
...
平台和 DSH 的新日志会以带 Source 的 JSON 实时写入容器 stdout 或 stderr，因此 `docker logs deepseek-harness` 可以查看完整运行流。两个管理界面支持按文本、Source 和级别筛选，可选择最近 100、250、500 或 1000 条处理后记录，并提供手动刷新、自动滚动开关和 JSONL 导出。每条日志默认显示简短摘要，展开后显示错误堆栈、Cause、任务 ID 和其他完整诊断字段。“清空显示”只影响当前浏览器视图，不删除日志文件。容器启动时不会向 stdout 重放历史日志。重复的认证、2FA、管理入口探测和上游故障会按失败类别限频：首条保留，短窗口内的重复项被抑制，下一条保留事件通过 `suppressedCount` 给出累计数。浏览器会话活动最多每个会话每五分钟记录一次；绝对过期、空闲过期、凭据或入口版本失效，以及来源 DSH 会话失效使用独立原因码。Root 认证恢复会同时留下操作类型、结果和会话撤销数量的审计记录，但不会记录密码、恢复 key、动态码、Cookie 或 CSRF token。`/data/platform/logs` 中按 Source 分离的 JSONL 是权威日志存储，默认按总量 100 MiB、保留 14 天自动轮转。
