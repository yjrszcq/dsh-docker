...
自动检查默认每六小时带抖动执行一次，可在任一管理前端中关闭或调整频率。检查不会自动下载或激活更新。可选提醒只在 DSH 页面中显示，且只由自动检查触发；独立管理中心不显示更新弹窗。打开任一管理前端时会在后台执行一次只读检查，手动检查也只刷新已保存结果，不触发提醒。两套界面均默认进入“运行维护”：独立管理中心后续依次为“文件管理、容器终端、系统插件、系统技能、用户插件、用户技能、更新管理”，DSH 内“平台管理”后续依次为“系统插件、系统技能、认证设置、代理设置、更新管理”。其认证标签只能撤销当前浏览器的 DSH Session 与关联 Management Session，不能修改凭据或取得完整 Management 权限。Management 组件通过 `/_dsh_platform/console/` 提供独立管理中心，并优先使用已保存的 DSH 语言。
