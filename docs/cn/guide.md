...
总代理开关与各来源开关相互独立。受管流量分为更新管理、平台组件、DSH 核心、DSH 插件、Agent 联网操作和管理中心容器终端。DSH 能在 `llm/stream` 中识别的远程模型 Provider 均可单独配置：代理开关关闭时直连；开启后可选择独立代理，或高亮“**跟随 DSH**”以使用 DSH 共享流量策略。共享策略在 DSH 核心或 DSH 插件任一范围启用时使用代理。本地和 loopback Provider 始终直连；若未来出现无法端到端携带稳定 Provider 身份的客户端，界面会锁定“跟随 DSH”并给出原因。
...
自动检查默认每六小时带抖动执行一次，可在任一管理前端中关闭或调整频率。检查不会自动下载或激活更新。可选提醒只在 DSH 页面中显示，且只由自动检查触发；独立管理中心不显示更新弹窗。打开任一管理前端时会在后台执行一次只读检查，手动检查也只刷新已保存结果，不触发提醒。两套界面均默认进入“运行维护”：独立管理中心后续依次为“文件管理、容器终端、系统插件、系统技能、用户插件、用户技能、更新管理”，DSH 内“平台管理”后续依次为“系统插件、系统技能、更新管理”。Management 组件通过 `/_dsh_platform/console/` 提供独立管理中心，并优先使用已保存的 DSH 语言。
