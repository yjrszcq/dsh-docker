...
Repository or organization Secrets `GOTIFY_URL` and `GOTIFY_TOKEN` are passed explicitly to the reusable Gotify workflow after both image variants are published and verified. Signing and Environment Release do not send an intermediate success notification, so a notification outage cannot prevent the independent image workflow from starting.
