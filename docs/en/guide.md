...
On a normal cold start, Bootstrap completes update and User Plugin recovery preflight, then starts the supervised DSH Runtime concurrently with the independent Control Plane startup group. Gateway can serve its authenticated holding and recovery surfaces before DSH is usable. DSH listening is not public readiness: Bootstrap publishes the current lifecycle generation only after DSH resources and Plugin inventory are verified and the complete Control Plane, access classification, Management, and recovery state are ready. Gateway admits DSH HTTP and WebSocket traffic only for that published generation, so neither a Gateway-first nor a DSH-first startup race can expose a half-ready Runtime or bypass Gateway.
...
A successful DSH login creates an HttpOnly, SameSite DSH Session. It does not authorize the complete Management API. Opening the Management Console consumes a one-time handoff and creates a separate Management Session bound to its Origin and source DSH Session. Direct compatibility or isolated-Origin access always returns to the verified DSH Origin for the main login first; an enabled Management console password is a second layer, never a replacement for the main login. Browser forms only transport values; the Access Manager performs authentication and credential decisions. Authentication Settings changes the username without a current password, while main-password and Management-console-password changes require the current main password; the old Management console password is not requested. Resetting or disabling that password revokes existing Management Sessions. DSH logout, expiry, or credential invalidation transitively invalidates linked Management Sessions. Authentication Settings can independently revoke either session class, and changing credentials invalidates sessions through the credential version.

The Access Manager counts password failures per browser source. The fifth consecutive failure starts a 30-second retry wait; each later admitted failure doubles it to a 15-minute maximum. Each source also has fixed rolling limits of 12 failures per hour and 24 per 24 hours. Across all sources, the whole DSH instance has a wider flood limit of 20 failures per minute, 60 per hour, and 120 per 24 hours. A successful password check clears that source's consecutive and rolling history but not instance-wide history. Gateway renders the backend error and remaining whole seconds but never makes the admission decision. Access Manager restart clears these in-memory counters.

When DSH is in a classified stopped, starting, recovering, or failed state, a top-level browser navigation receives a generic Gateway holding page before authentication. It exposes no runtime error detail and links to Management for recovery. Non-page, API, and WebSocket requests continue to return the classified service failure instead of receiving HTML. Once DSH is ready, normal page access still requires a valid DSH Session.

The default compatibility mode serves Management at `/_dsh_platform/console/` on port `3080`. Optional strong isolation serves Management from a separate container entry with no DSH upstream: a public separate origin uses the exact public Origin, while a local separate origin uses only the host port mapped to that entry (default `3081`, configurable to any valid port). The platform verifies the exact candidate entry and instance before switching, then revokes prior Management sessions. The supplied Compose does not publish the separate entry unless the operator explicitly maps or reverse-proxies it. When DSH can obtain container Root, the UI honestly reports that process-level isolation is ineffective and locks this mode transition; Origin separation still protects browser sessions from same-origin DSH client plugins.
...
`access reset` is the recommended combined recovery command. It independently asks whether to change the username and main password. When a Management console password is enabled, `y` replaces it, the default `n` preserves it, and `d` disables it. When two-factor authentication is enabled, a separate `y/[n]` prompt can disable it. Nothing is saved until every selected prompt is complete, so cancelling or losing the TTY halfway through does not partially update the account. Yes/no recovery prompts accept only `y` or `n` and show their default in brackets. The narrower interactive commands remain available for single-credential recovery. Session invalidation follows the selected changes: resetting the main password clears all sessions, setting or resetting the Management console password clears Management sessions, and disabling either optional layer keeps existing sessions.
...
The master switch and each source switch are independent. Managed traffic is divided into updates, platform components, DSH core, DSH plugins, Agent network operations, and the Management container terminal. Every remote model Provider that DSH identifies in `llm/stream` can be configured independently: turning its proxy switch off selects direct access, while turning it on uses a dedicated proxy route. Loopback and other local Providers are always direct. If a future client cannot carry a stable Provider identity end to end, the UI shows a read-only **Follow DSH** label and an explanation; its switch selects direct access when off and the shared DSH policy when on.
...
Before a registered operation disconnects DSH, open browser pages move to a localized holding page. The page distinguishes starting, stopping, stopped, restarting, unexpected recovery, Runtime switching/recovery, and startup failure, then returns to the original same-origin path after readiness. Gateway readiness requires Bootstrap to publish the current lifecycle generation after both a responsive DSH HTTP upstream and the complete platform lifecycle have left every start, restart, recovery, and switch state. It therefore does not return the browser while DSH is listening but Bootstrap is still completing resource, Plugin inventory, Control Plane, or recovery checks. A brief transport interruption is verified through this combined readiness before navigation. API and WebSocket requests continue receiving `503`; these classified lifecycle responses are not logged as upstream failures or recoveries. Unknown proxy failures remain `502` and retain their failure logs.

If a browser requests a plugin bundle during a registered lifecycle transition, Gateway holds the request until DSH is ready. This applies to every DSH-loaded client bundle, including official DSH plugins, bundled System Plugins, and User Plugins. If a bundle request still returns `502` or `503`, a network failure reaches the dynamic importer, or the importer raises a structured module-load error, the guard confirms platform readiness and may enter the holding page twice for the same running Deployment. A third structured failure opens Gateway's terminal **DSH plugins keep failing to load** page with a link to the standalone Management Console; it does not create an unbounded reload loop. Detection uses the failed plugin request or module error URL, never visible page text. The platform log records failure, recovery start, recovery completion, and final failure as `browser.plugin-load.failed`, `browser.plugin-load.recovery.started`, `browser.plugin-load.recovery.completed`, and `browser.plugin-load.recovery.failed`, including bounded plugin, revision, lifecycle-task, recovery-attempt, and reason fields.
...
The standalone console lists every System Plugin bundled by the current Environment and can install, uninstall, enable, or disable them, including recovery of the `platform-management` DSH integration. The integration inside DSH shows missing plugins with an Install action and limits installed plugins to enable/disable; it cannot uninstall them. Selections remain a page-local draft until Apply, so refreshing or leaving before Apply discards them. Apply persists each change and normally restarts DSH to activate it. If DSH is already unavailable, or that restart fails after the change commits, the stored selection is retained for the next successful start instead of remaining falsely marked as unapplied. Installation rebuilds and verifies the complete System Plugin Set from the current Deployment's local trusted Environment Artifact against the Deployment Record content hash. It never contacts GitHub or npm, never copies files from a built Runtime, and never reinstalls a missing plugin automatically.
...
New Platform and DSH log entries are also emitted as source-tagged JSON to container stdout or stderr, so `docker logs deepseek-harness` shows the complete live operational stream. Both Management interfaces support text, Source, and level filters; processed-entry limits of 100, 250, 500, or 1000; manual refresh; optional automatic scrolling; and JSONL export. Each row starts with a compact summary and expands to the complete structured record, including error stacks, causes, task IDs, and diagnostic fields. **Clear view** affects only the current browser view and does not delete stored logs. Historical entries are not replayed to stdout at startup. Repeated authentication, 2FA, Management-origin probe, and upstream failures are rate-limited by failure class: the first event is retained, repetitions in the short window are suppressed, and the next retained event includes `suppressedCount`. Browser-session activity is emitted at most once per session every five minutes; absolute expiry, idle expiry, credential or entry-version invalidation, and source-DSH-session invalidation use distinct reason codes. Root authentication recovery also writes audit records containing the operation, result, and session-revocation counts, but never passwords, reset keys, one-time codes, Cookies, or CSRF tokens. Source-separated JSONL under `/data/platform/logs` remains the authoritative store and rotates according to `DSH_LOG_MAX_BYTES` and `DSH_LOG_RETENTION_DAYS` (100 MiB and 14 days by default).
...
Changing channels modifies only local desired state. Stable converges to the signed supported DSH and Environment. Experimental first converges the complete Deployment to the signed Stable target sequence, even when its visible DSH and Environment versions already match, then offers the newest verified upstream DSH. When current DSH is ahead of Latest Supported, the complete combination is frozen until Stable catches up.

Candidate build failures create a version Hold; incompatible Runtime/Environment combinations create a combination Hold. If npm exposes the selected DSH version before every dependency version is available, materialization automatically retries the transient `E404`, `ETARGET`, or `ENOVERSIONS` failure in fresh install roots with bounded backoff. Only a failure that remains after those retries creates the version Hold, with a bounded registry reason. `retry` clears the one active Hold or Blocked combination after the registry graph or compatibility issue has been corrected.
