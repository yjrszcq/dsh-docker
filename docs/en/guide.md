...
The master switch and each source switch are independent. Managed traffic is divided into updates, platform components, DSH core, DSH plugins, Agent network operations, and the Management container terminal. Every remote model Provider that DSH identifies in `llm/stream` can be configured independently: turning its proxy switch off selects direct access; turning it on can use either an independent proxy route or the shared DSH policy when **Follow DSH** is highlighted. The shared policy uses the proxy when either DSH Core or DSH Plugins is enabled. Loopback and other local Providers are always direct. If a future client cannot carry a stable Provider identity end to end, the UI locks **Follow DSH** and explains why.
