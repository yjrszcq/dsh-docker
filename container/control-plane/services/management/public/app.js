...
const COPY = Object.freeze({
  zh: Object.freeze({
    ...
    authUsernameFormat: '用户名支持 1 至 64 个字符，可使用中文、字母、数字、空格和常用符号；不能包含控制字符或双向控制字符。', authPasswordFormat: '密码支持 8 至 1024 个字符，可使用中文、字母、数字、空格和符号；不能包含控制字符或双向控制字符。', authCurrentPasswordRequired: '请输入当前主密码。', authCurrentPasswordIncorrect: '当前主密码不正确。', authManagementPasswordRequired: '请输入新的管理中心密码。', authLocalPortFormat: '请输入 1 至 65535 的宿主机端口。', authTotpTitle: '两步验证（2FA）', authTotpEnabled: '启用两步验证', authTotpSetupTitle: '设置两步验证', authTotpSetupDetail: '使用身份验证器扫描二维码，再输入当前动态验证码。', authTotpQrAlt: '两步验证二维码', authTotpShowSecret: '显示 2FA 密钥', authTotpHideSecret: '隐藏 2FA 密钥', authTotpCode: '动态验证码', authTotpConfirm: '确认并启用', authTotpCodeInvalid: '动态验证码不正确，请重试。', authTotpCodeRequired: '请输入 6 位动态验证码。', authTotpRetryRequired: '动态验证码连续多次错误，请在 {seconds} 秒后重试。', authTotpRateLimited: '动态验证码尝试次数过多，请在 {seconds} 秒后重试。', authTotpEnrollmentExpired: '二维码已失效，请关闭后重新启用。',
    ...
    proxyTest: '连接测试', proxyTestDetail: '使用当前表单测试，不保存也不激活配置。', proxyTestStart: '测试连接', proxySave: '保存并应用', proxySaving: '正在保存代理设置', proxySaved: '代理设置已保存并应用', proxyTestRunning: '正在测试代理连接', proxyTestSuccess: '代理连接测试通过', proxyTestFailed: '代理连接测试失败', proxyTestCancelled: '代理连接测试已取消', proxyStageAddress: '解析代理地址', proxyStageConnect: '连接代理服务器', proxyStageHandshake: '验证代理协议与认证', proxyStageDns: '解析目标地址', proxyStageTls: '验证目标 TLS', proxyStageHttp: '请求 GitHub 与 npm', proxyStagePending: '待测试', proxyStageRunning: '测试中', proxyStageSuccess: '通过', proxyStageFailed: '失败', proxyStageCancelled: '已取消', proxyStageSkipped: '无需执行', proxyStageDetailDirect: '当前候选配置使用直连。', proxyStageDetailAddress: '已解析 {count} 个代理地址记录。', proxyStageDetailConnect: '已成功连接代理服务器。', proxyStageDetailHandshake: '已验证 {protocol} 代理协议与认证。', proxyStageDetailDns: '已解析 {count} 个目标地址。', proxyStageDetailTls: '已验证 {count} 个目标的 TLS 连接。', proxyStageDetailHttp: '{results}', proxyTargetGithub: 'GitHub metadata', proxyTargetNpm: 'npm registry',
...
async function api(path, { method = 'GET', body } = {}) {
  ...
  if (!response.ok) {
    ...
    error.retryAfterSeconds = detail?.retryAfterSeconds ?? value.retryAfterSeconds
    throw error
  }
  return value
}
...
function localizedProxyTestDetail(stage, fallback) {
...
function clearProxySecrets() {
  ...
  elements['proxy-password'].disabled = proxyConfiguration?.componentReady === false
  elements['proxy-clear-password'].setAttribute('aria-pressed', 'false')
...
function renderProxyProviders() {
  ...
  for (const provider of providers) {
    ...
    input.disabled = provider.routingCapability === 'forced-direct' || proxyConfiguration.componentReady === false
...
function renderProxyConfiguration() {
  ...
  elements['proxy-enabled'].disabled = componentUnavailable
  ...
  for (const id of [
    ...
    'proxy-clear-password', 'proxy-advanced-toggle', 'proxy-direct-rules',
    'proxy-system-rules', 'proxy-remote-dns', 'proxy-provider-search', 'proxy-scope-help',
  ]) elements[id].disabled = componentUnavailable
  for (const input of document.querySelectorAll('[data-proxy-scope]')) {
    input.checked = configuration.scopes?.[input.dataset.proxyScope] === true
    input.disabled = componentUnavailable
  }
  ...
  elements['proxy-all-proxy'].disabled = componentUnavailable
  elements['proxy-test'].disabled = componentUnavailable
  elements['proxy-save'].disabled = componentUnavailable
  elements['proxy-component-alert'].hidden = !componentUnavailable
...
function renderProxyTest(task = proxyTestTask) {
  ...
  for (const stage of task?.stages ?? []) {
    ...
    detail.textContent = localizedProxyTestDetail(stage, status)
  ...
  const componentUnavailable = proxyConfiguration?.componentReady === false
  elements['proxy-test'].disabled = running || componentUnavailable
  elements['proxy-save'].disabled = running || componentUnavailable
...
async function saveProxyConfiguration() {
  ...
  try {
    ...
    elements['proxy-save'].disabled = proxyConfiguration?.componentReady === false
  }
}
...
function restoreProxyDraft(draft) {
  ...
  elements['proxy-password'].disabled = draft.clearPassword === 'true'
    || proxyConfiguration?.componentReady === false
...
const AUTHENTICATION_PASSWORD_FIELDS = Object.freeze([
  ...
  'auth-additional-password-confirm',
])

function renderAuthenticationFormatError(id) {
  const input = elements[id]
  const error = elements[`${id}-error`]
...
function authenticationAccountDraft() {
  ...
  const totpEnabled = elements['auth-totp-enabled'].checked
  const priorAdditionalEnabled = authenticationSettings?.account?.managementAdditionalCredential?.enabled === true
  const priorTotpEnabled = authenticationSettings?.account?.totp?.enabled === true
  ...
  return {
    ...
    totpEnabled,
    additionalPassword,
    priorAdditionalEnabled,
    priorTotpEnabled,
    usernameChanged: username !== (authenticationSettings?.account?.username ?? ''),
    mainPasswordChanged: password !== '',
    additionalChanged: additionalEnabled !== priorAdditionalEnabled || additionalPassword !== '',
    totpChanged: totpEnabled !== priorTotpEnabled,
  }
}

function validateAdditionalPassword(draft) {
  if (!draft.additionalEnabled || draft.priorAdditionalEnabled || draft.additionalPassword !== '') return true
  const error = elements['auth-additional-password-error']
  error.textContent = t('authManagementPasswordRequired')
  error.hidden = false
  return false
}

function accountDraftChanged(draft = authenticationAccountDraft()) {
  return draft.usernameChanged || draft.mainPasswordChanged || draft.additionalChanged || draft.totpChanged
}

function renderAuthenticationAccountSaveState() {
  elements['auth-account-save'].disabled = !accountDraftChanged()
}
...
function validateCurrentPassword(draft, { incorrect = false } = {}) {
  ...
  if (!draft.mainPasswordChanged && !draft.additionalChanged && !draft.totpChanged) {
    error.hidden = true
    return true
  }
  if (input.value === '') {
    error.textContent = t('authCurrentPasswordRequired')
    error.hidden = false
    return false
  }
  if (incorrect) {
    error.textContent = t('authCurrentPasswordIncorrect')
    error.hidden = false
    return false
  }
...
function detectedManagementOrigin() {
  const value = new URL(window.location.origin)
  value.port = '3081'
  return value.origin
}

function accessMode(value) {
  return value === 'compat' ? 'compat' : 'isolated'
}

function renderAdditionalPasswordFields() {
  const enabled = elements['auth-additional-enabled'].checked
  elements['auth-additional-fields'].hidden = !enabled
  if (!enabled) {
    for (const id of ['auth-additional-password', 'auth-additional-password-confirm']) {
      elements[id].value = ''
      elements[`${id}-error`].hidden = true
    }
  }
}

function browserDevice(userAgent) {
  if (typeof userAgent !== 'string' || userAgent === '') return t('authSessionUnknownDevice')
  const browser = /Edg\//.test(userAgent) ? 'Edge'
    : /Firefox\//.test(userAgent) ? 'Firefox'
      : /Chrome\//.test(userAgent) ? 'Chrome'
        : /Safari\//.test(userAgent) ? 'Safari' : t('authSessionUnknownDevice')
...
function renderAuthenticationSessions(sessions) {
  const list = elements['auth-session-list']
  ...
  for (const session of sessions) {
    ...
    const heading = document.createElement('div')
    heading.className = 'auth-session-heading'
    const device = document.createElement('span')
    device.className = 'auth-session-device'
    device.textContent = browserDevice(session.userAgent)
    if (typeof session.userAgent === 'string') device.title = session.userAgent
    heading.append(device)
    if (session.current === true) {
      const current = document.createElement('span')
      current.className = 'auth-session-current'
    ...
    for (const [key, value] of [
      ['authSessionIp', session.ip ?? '—'],
      ['authSessionLoginAt', localTime(session.createdAt)],
      ['authSessionLastSeen', localTime(session.lastSeenAt)],
    ]) {
      const field = document.createElement('span')
      field.textContent = t(key, { value })
      meta.append(field)
    }
    details.append(heading, meta)
    const button = document.createElement('button')
    button.type = 'button'
    button.className = 'auth-session-revoke secondary'
    button.dataset.sessionId = session.sessionId
    const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg')
    svg.setAttribute('viewBox', '0 0 24 24')
    svg.setAttribute('aria-hidden', 'true')
    for (const pathData of ['M10 17l5-5-5-5', 'M15 12H3', 'M15 4h4a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2h-4']) {
      const path = document.createElementNS('http://www.w3.org/2000/svg', 'path')
      path.setAttribute('d', pathData)
      svg.append(path)
    }
    const label = document.createElement('span')
    label.textContent = t('authSessionRevoke')
    button.append(svg, label)
    button.addEventListener('click', () => { void revokeAuthenticationSession(button) })
...
async function loadAuthenticationSettings() {
  try {
    ...
    renderAuthenticationSessions(sessions)
    ...
    elements['auth-username-error'].hidden = true
    elements['auth-settings-status'].hidden = true
...
function renderAuthenticationOrigin() {
  ...
  if (rootEnabled) elements['auth-root-warning'].textContent = t('authRootLocked')
...
function confirmIsolationEnable() {
  const dialog = elements['auth-isolation-confirm-dialog']
...
async function saveAccountSettings() {
  ...
  if (!formatValid || !additionalPasswordValid || !currentPasswordValid || !confirmationsValid) return
  const button = elements['auth-account-save']
  button.disabled = true
  try {
    const body = { username: draft.username, currentPassword: elements['auth-current-password'].value }
    ...
    if (draft.totpChanged && draft.totpEnabled) {
      ...
      openTotpEnrollment(enrollment, body)
      return
    }
...
function finishAccountSettings(result) {
  const button = elements['auth-account-save']
  if (result.currentManagementSessionRevoked === true) {
    ...
    authenticationSettings.account = result.account
    button.disabled = false
    void loadAuthenticationSettings().then(() => showAuthenticationStatus(t('authSaved')))
  }
}

function resetTotpDialog() {
  clearTimeout(totpRetryTimer)
  elements['auth-totp-qr'].removeAttribute('src')
  elements['auth-totp-secret'].textContent = ''
  elements['auth-totp-secret-row'].hidden = true
  elements['auth-totp-secret-toggle'].setAttribute('aria-expanded', 'false')
  elements['auth-totp-secret-toggle'].textContent = t('authTotpShowSecret')
  elements['auth-totp-code'].value = ''
  elements['auth-totp-code'].disabled = false
  elements['auth-totp-error'].hidden = true
  elements['auth-totp-confirm'].disabled = false
}

function openTotpEnrollment(enrollment, body) {
  resetTotpDialog()
  pendingTotpEnrollment = { enrollment, body }
...
async function cancelTotpEnrollment() {
  ...
  try {
    ...
    elements['auth-totp-error'].textContent = error.message
...
function showTotpRetryCountdown(error) {
  const field = elements['auth-totp-error']
  const input = elements['auth-totp-code']
  const button = elements['auth-totp-confirm']
  const template = t(error.code === 'TOTP_RATE_LIMITED' ? 'authTotpRateLimited' : 'authTotpRetryRequired')
  const retryUntil = Date.now() + error.retryAfterSeconds * 1000
  input.disabled = true
  button.disabled = true
  const tick = () => {
    const seconds = Math.max(0, Math.ceil((retryUntil - Date.now()) / 1000))
    if (seconds > 0) {
      field.textContent = template.replace('{seconds}', String(seconds))
      field.hidden = false
      totpRetryTimer = window.setTimeout(tick, Math.min(1000, Math.max(1, retryUntil - Date.now())))
      return
    }
    field.hidden = true
...
async function confirmTotpEnrollment() {
  ...
  if (!/^\d{6}$/u.test(code)) {
    elements['auth-totp-error'].textContent = t('authTotpCodeRequired')
    elements['auth-totp-error'].hidden = false
    return
  }
  elements['auth-totp-confirm'].disabled = true
  try {
    const result = await api('auth-settings', {
      method: 'PUT',
      body: {
        ...pending.body,
        totpEnabled: true,
        totpEnrollmentToken: pending.enrollment.enrollmentToken,
        totpCode: code,
      },
    })
    pendingTotpEnrollment = undefined
    ...
    if (error.code === 'TOTP_INVALID') {
      elements['auth-totp-error'].textContent = t('authTotpCodeInvalid')
      elements['auth-totp-error'].hidden = false
      elements['auth-totp-code'].select()
      elements['auth-totp-confirm'].disabled = false
      ...
      elements['auth-totp-error'].hidden = false
    } else if (error.code === 'FRESH_AUTH_FAILED') {
      await cancelTotpEnrollment()
      validateCurrentPassword(authenticationAccountDraft(), { incorrect: true })
      elements['auth-current-password'].focus()
    } else {
      elements['auth-totp-confirm'].disabled = false
      showError(error)
...
async function revokeAuthenticationSession(button) {
  const buttons = [...document.querySelectorAll('.auth-session-revoke')]
  for (const value of buttons) value.disabled = true
  try {
    const result = await api('auth-sessions/revoke', {
      method: 'POST',
      body: { sessionId: button.dataset.sessionId },
    })
...
elements['proxy-component-alert'].addEventListener('click', () => elements['proxy-component-dialog'].showModal())
elements['proxy-component-dialog'].addEventListener('click', event => {
  if (event.target === event.currentTarget) event.currentTarget.close()
})
elements['proxy-provider-info-dialog'].addEventListener('click', event => {
  if (event.target === event.currentTarget) event.currentTarget.close()
})
...
elements['auth-isolation-compare'].addEventListener('click', () => elements['auth-isolation-dialog'].showModal())
elements['auth-isolation-dialog'].addEventListener('click', event => {
  if (event.target === event.currentTarget) event.currentTarget.close()
})
for (const id of ['auth-password', 'auth-additional-password']) {
...
for (const [passwordId, confirmationId] of [
  ['auth-password', 'auth-password-confirm'],
  ['auth-additional-password', 'auth-additional-password-confirm'],
]) {
...
elements['auth-totp-enabled'].addEventListener('change', renderAuthenticationAccountSaveState)
elements['auth-totp-secret-toggle'].addEventListener('click', () => {
  const expanded = elements['auth-totp-secret-toggle'].getAttribute('aria-expanded') === 'true'
  elements['auth-totp-secret-toggle'].setAttribute('aria-expanded', String(!expanded))
...
elements['auth-totp-code'].addEventListener('input', () => {
  if (!elements['auth-totp-code'].disabled) elements['auth-totp-error'].hidden = true
})
elements['auth-settings-form'].addEventListener('submit', event => { event.preventDefault() })
elements['auth-account-save'].addEventListener('click', () => { void saveAccountSettings() })
elements['auth-origin-save'].addEventListener('click', () => { void saveManagementOrigin() })
...
window.setInterval(() => { void api('status').catch(() => {}) }, 5_000)
window.setInterval(() => {
