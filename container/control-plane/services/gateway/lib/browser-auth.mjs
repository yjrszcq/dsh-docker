...
function authenticationPage(request, state, csrf, returnPath) {
  ...
  const copy = zh ? {
    ...
    invalidPassword: '密码必须包含 8 至 1024 个字符，且不能包含控制字符或双向控制字符。',
    ...
    invalidPassword: 'The password must contain between 8 and 1024 characters and cannot contain control or bidirectional-control characters.',
