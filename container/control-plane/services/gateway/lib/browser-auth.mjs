...
function managementAuthenticationContext(request, origin, managementPath = '/_dsh_platform/') {
  const csrfToken = cookieValue(request.headers.cookie, MANAGEMENT_CSRF_COOKIE) ?? token('dshm')
  return {
    csrfToken,
    cookies: [`${MANAGEMENT_CSRF_COOKIE}=${csrfToken}; SameSite=Strict; Path=${managementPath}${secureCookie(origin)}`],
  }
}
...
function authenticationPage(request, state, csrf, authenticationContextId, returnPath, { resetForm = false } = {}) {
  ...
  const copy = zh ? {
    brand: 'HARNESS', username: '用户名', password: '密码', confirmPassword: '确认密码', passwordMismatch: '两次输入的新密码不一致。', totp: '动态验证码', register: '注册', login: '登录',
    ...
    brand: 'HARNESS', username: 'Username', password: 'Password', confirmPassword: 'Confirm password', passwordMismatch: 'The new password confirmation does not match.', totp: 'Authentication code', register: 'Register', login: 'Sign in',
  ...
  if (state === 'never-initialized' || state === 'initialized' || state === 'migration-required'
    ...
    const passwordConfirmation = validateCredentials
      ? `<label data-credential-field>${copy.confirmPassword}<input name="confirmPassword" type="password" autocomplete="new-password" required maxlength="1024"><small class="field-error" data-field-error="confirmPassword" role="alert" hidden>${htmlEscape(copy.passwordMismatch)}</small></label>` : ''
    const validationScript = validateCredentials
      ? `function validateField(name){const input=form.elements[name],message=form.querySelector('[data-field-error="'+name+'"]'),valid=input.validity.valid;message.hidden=valid;return valid}function validatePasswordConfirmation(show=false){const input=form.elements.confirmPassword,message=form.querySelector('[data-field-error="confirmPassword"]'),matches=input.value===form.elements.password.value;message.hidden=matches||(!show&&input.value.length===0);return matches}form.elements.password.addEventListener('input',()=>{validateField('password');validatePasswordConfirmation()});form.elements.confirmPassword.addEventListener('input',()=>validatePasswordConfirmation());`
      : ''
    ...
    const validationGate = validateCredentials
      ? `validateField('username');validateField('password');if(!validatePasswordConfirmation(true))return;` : ''
    ...
    content = `${context}<form method="post" action="${htmlEscape(submitPath)}" novalidate>${reset ? `<label>${copy.setupKey}<input name="setupKey" type="password" autocomplete="one-time-code" required maxlength="512" autofocus></label>` : ''}<label data-credential-field>${copy.username}<input name="username" autocomplete="username" maxlength="256"${usernameValidation}${reset ? '' : ' autofocus'}>${usernameError}</label><label data-credential-field>${copy.password}<input name="password" type="password" autocomplete="${validateCredentials ? 'new-password' : 'current-password'}" maxlength="1024"${passwordValidation}>${passwordError}</label>${passwordConfirmation}${totpField}<button type="submit">${submit}</button><p class="error" role="alert" hidden></p></form><script>const form=document.querySelector('form'),error=document.querySelector('.error'),button=form.querySelector('button'),totpField=document.querySelector('#totp-field'),failureMessages=${JSON.stringify(registrationErrors)},retryMessage=${JSON.stringify(copy.retryRequired)},rateLimitMessage=${JSON.stringify(copy.rateLimited)},totpRetryMessage=${JSON.stringify(copy.totpRetryRequired)},totpRateLimitMessage=${JSON.stringify(copy.totpRateLimited)};let navigating=false,csrfToken=${JSON.stringify(csrf)},authenticationContextId=${JSON.stringify(authenticationContextId)},totpPending=false,totpRetryRefreshTimer;${authenticationCountdownScript(submit)}${validationScript}function clearRetryCountdown(){clearTimeout(retryTimer);retryUntil=0;error.textContent='';error.hidden=true;button.disabled=false;button.textContent=${JSON.stringify(submit)}}async function refreshTotpRetry(){clearTimeout(totpRetryRefreshTimer);if(!totpPending||retryUntil<=Date.now())return;try{const response=await fetch(${JSON.stringify(`${AUTH_PREFIX}context`)},{headers:{accept:'application/json'},cache:'no-store'}),next=await response.json().catch(()=>({}));if(response.ok&&typeof next.csrfToken==='string'&&typeof next.authenticationContext==='string'){csrfToken=next.csrfToken;authenticationContextId=next.authenticationContext;const retry=next.totpRetry;if(retry&&Number.isInteger(retry.retryAfterSeconds)){if(retry.retryAfterSeconds<=0){clearRetryCountdown();return}const template=retry.kind==='rate'?totpRateLimitMessage:totpRetryMessage;if(Math.abs(retryUntil-Date.now()-retry.retryAfterSeconds*1000)>1500)showRetryCountdown(template,retry.retryAfterSeconds)}}}catch{}if(totpPending&&retryUntil>Date.now())totpRetryRefreshTimer=setTimeout(refreshTotpRetry,1000)}function showTotpRetry(payload){showRetryCountdown(payload.code==='TOTP_RATE_LIMITED'?totpRateLimitMessage:totpRetryMessage,payload.retryAfterSeconds);totpRetryRefreshTimer=setTimeout(refreshTotpRetry,1000)}function showTotp(){totpPending=true;for(const field of form.querySelectorAll('[data-credential-field]'))field.hidden=true;totpField.hidden=false;form.elements.password.value='';error.textContent=${JSON.stringify(copy.totpRequired)};error.hidden=false;form.elements.totpCode.focus()}function resetTotp(message){totpPending=false;clearTimeout(totpRetryRefreshTimer);for(const field of form.querySelectorAll('[data-credential-field]'))field.hidden=false;totpField.hidden=true;form.elements.totpCode.value='';error.textContent=message;error.hidden=false;form.elements.username.focus()}async function submitAuthentication(values,retry=true){const requestBody=totpPending?{totpCode:values.get('totpCode')}:{${reset ? "setupKey:values.get('setupKey')," : ''}username:values.get('username'),password:values.get('password')},body=JSON.stringify({...requestBody,authenticationContext:authenticationContextId}),response=await fetch(${JSON.stringify(submitPath)},{method:'POST',headers:{'content-type':'application/json','x-dsh-csrf':csrfToken},body});const payload=await response.json().catch(()=>({}));if(response.status===409&&payload.code==='AUTHENTICATION_CONTEXT_STALE'&&retry&&!totpPending){const refreshed=await fetch(${JSON.stringify(`${AUTH_PREFIX}context`)},{headers:{accept:'application/json'},cache:'no-store'}),next=await refreshed.json().catch(()=>({}));if(refreshed.ok&&typeof next.csrfToken==='string'&&typeof next.authenticationContext==='string'){csrfToken=next.csrfToken;authenticationContextId=next.authenticationContext;return submitAuthentication(values,false)}}return{response,payload}}form.addEventListener('submit',async event=>{event.preventDefault();error.hidden=true;if(!totpPending){${validationGate}}button.disabled=true;button.textContent=${JSON.stringify(initialize ? copy.initializing : reset ? copy.resetting : copy.signingIn)};const values=new FormData(form);try{const{response,payload}=await submitAuthentication(values);if(response.ok&&payload.totpRequired===true){showTotp();return}if(response.ok){navigating=true;location.replace(typeof payload.next==='string'?payload.next:${JSON.stringify(returnPath)});return}if(totpPending){if(payload.code==='TOTP_LOGIN_INVALID'||payload.code==='AUTHENTICATION_CONTEXT_STALE'){resetTotp(${JSON.stringify(copy.loginUpdated)})}else if(payload.code==='TOTP_RETRY_REQUIRED'||payload.code==='TOTP_RATE_LIMITED'){showTotpRetry(payload)}else{error.textContent=${JSON.stringify(copy.totpFailed)};error.hidden=false;form.elements.totpCode.select()}}else if(!showRetryFailure(payload)){error.textContent=failureMessages[payload.code]??${JSON.stringify(fallbackError)};error.hidden=false}}catch{error.textContent=${JSON.stringify(copy.serviceUnavailable)};error.hidden=false}finally{if(!navigating&&retryUntil<=Date.now())button.disabled=false;if(!navigating)button.textContent=${JSON.stringify(submit)}}})</script>`
  ...
  } else content = `<section class="waiting"><h1>${copy.waitingTitle}</h1><p>${copy.waitingDetail}</p></section><script>setTimeout(()=>location.reload(),1000)</script>`
  return `<!doctype html><html lang="${zh ? 'zh-CN' : 'en'}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${htmlEscape(copy.brand)}</title><style>html{height:100%;color-scheme:light dark}*{box-sizing:border-box}body{min-height:100%;margin:0;display:grid;place-items:center;background:#151517;color:#f3f3f4;font:14px/1.5 Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.card{width:min(400px,calc(100% - 32px));padding:28px;border:1px solid #3e3e42;border-radius:12px;background:#242426}.brand{text-align:center;margin:0 0 24px;font-size:18px;letter-spacing:.12em}h1{margin:0 0 8px;font-size:20px}p{margin:0;color:#aaaab0}.waiting{text-align:center}.detail{margin-bottom:20px}form{display:grid;gap:16px}label{display:grid;gap:7px;font-weight:600}.field-error{color:#ff7777;font-size:12px;font-weight:400;line-height:1.5}input{width:100%;padding:10px 12px;border:1px solid #55555b;border-radius:7px;background:#19191b;color:inherit;font:inherit}button,.button{display:block;width:100%;padding:10px 14px;border:0;border-radius:7px;background:#f2f2f3;color:#202124;font:600 14px/1.4 inherit;text-align:center;text-decoration:none;cursor:pointer}input:focus-visible{border-color:#f3f3f4;outline:none}button:focus-visible,.button:focus-visible{outline:2px solid #6ea8fe;outline-offset:2px}.error{color:#ff7777}[hidden]{display:none}@media(prefers-color-scheme:light){body{background:#f7f7f8;color:#202124}.card{background:#fff;border-color:#d7d7da}p{color:#6d6f76}input{background:#fff;border-color:#c7c8cc}button,.button{background:#202124;color:#fff}}</style></head><body><main class="card"><div class="brand">${htmlEscape(copy.brand)}</div>${content}</main></body></html>`
}
...
function managementLoginPage(request, csrf, authenticationContextId, mainCredentialVersion, {
  ...
  return `<!doctype html><html lang="${zh ? 'zh-CN' : 'en'}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${copy.title}</title><style>html{height:100%;color-scheme:light dark}*{box-sizing:border-box}body{min-height:100%;margin:0;display:grid;place-items:center;background:#151517;color:#f3f3f4;font:14px/1.5 Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.card{width:min(400px,calc(100% - 32px));padding:28px;border:1px solid #3e3e42;border-radius:12px;background:#242426}h1{margin:0 0 6px;font-size:20px}p{margin:0 0 22px;color:#aaaab0}form{display:grid;gap:16px}label{display:grid;gap:7px;font-weight:600}input{width:100%;padding:10px 12px;border:1px solid #55555b;border-radius:7px;background:#19191b;color:inherit;font:inherit}button{padding:10px 14px;border:0;border-radius:7px;background:#f2f2f3;color:#202124;font:600 14px/1.4 inherit;cursor:pointer}input:focus-visible{border-color:#f3f3f4;outline:none}button:focus-visible{outline:2px solid #6ea8fe;outline-offset:2px}.error{color:#ff7777}[hidden]{display:none}@media(prefers-color-scheme:light){body{background:#f7f7f8;color:#202124}.card{background:#fff;border-color:#d7d7da}p{color:#6d6f76}input{background:#fff;border-color:#c7c8cc}button{background:#202124;color:#fff}}</style></head><body><main class="card"><h1>${copy.title}</h1><p>${copy.detail}</p><form method="post" action="${htmlEscape(submitPath)}" novalidate><label>${copy.password}<input name="password" type="password" autocomplete="current-password" maxlength="1024" autofocus></label><button type="submit">${copy.submit}</button><p class="error" role="alert" hidden>${copy.failed}</p></form></main><script>const form=document.querySelector('form'),error=document.querySelector('.error'),button=form.querySelector('button'),retryMessage=${JSON.stringify(copy.retryRequired)},rateLimitMessage=${JSON.stringify(copy.rateLimited)},mainCredentialVersion=${JSON.stringify(mainCredentialVersion)};let navigating=false,csrfToken=${JSON.stringify(csrf)},authenticationContextId=${JSON.stringify(authenticationContextId)};${authenticationCountdownScript(copy.submit)}async function submitAuthentication(password){const body=JSON.stringify({password,authenticationContext:authenticationContextId}),response=await fetch(${JSON.stringify(submitPath)},{method:'POST',headers:{'content-type':'application/json','x-dsh-csrf':csrfToken},body});const payload=await response.json().catch(()=>({}));if(response.status===409&&payload.code==='AUTHENTICATION_CONTEXT_STALE'){location.replace(${JSON.stringify(`${authPrefix}management${managementTabSuffix(searchParams)}`)});return new Promise(()=>{})}return{response,payload}}form.addEventListener('submit',async event=>{event.preventDefault();error.hidden=true;button.disabled=true;const values=new FormData(form);try{const{response,payload}=await submitAuthentication(values.get('password'));if(response.ok){navigating=true;location.replace(${JSON.stringify(destination)});return}if(payload.code==='PENDING_LOGIN_INVALID'){navigating=true;location.replace(${JSON.stringify(`${authPrefix}management${managementTabSuffix(searchParams)}`)});return}if(!showRetryFailure(payload)){error.textContent=payload.code==='ACCESS_MANAGER_UNAVAILABLE'?${JSON.stringify(copy.serviceUnavailable)}:${JSON.stringify(copy.failed)};error.hidden=false;form.elements.password.select()}}catch{error.textContent=${JSON.stringify(copy.serviceUnavailable)};error.hidden=false}finally{if(!navigating&&retryUntil<=Date.now())button.disabled=false}})</script></body></html>`
}
...
export function createBrowserAuthentication({
  ...
  async function sendManagementLogin(request, response, origin, searchParams) {
    const authContext = managementAuthenticationContext(request, origin, managementCookiePath)
  ...
  async function handle(request, response, pathname, searchParams) {
    ...
    if (pathname === authPrefix + 'context' && request.method === 'GET') {
      ...
      const current = await authenticationStatus()
      const totpRetry = current.state === 'initialized' && current.account?.totp?.enabled === true
        ? await accessRequest('POST', '/v1/authentication-retry/status', {
            credential: 'totp', source: authenticationSource(request),
          })
        : { kind: null, retryAfterSeconds: 0 }
    ...
    if (pathname === authPrefix + 'management/pending' && request.method === 'POST') {
      const csrf = cookieValue(request.headers.cookie, MANAGEMENT_CSRF_COOKIE)
      if (!validBrowserMutationShape(request)) {
        sendJson(response, 403, { error: 'authentication request rejected', code: 'REQUEST_FORBIDDEN' })
        return true
      }
