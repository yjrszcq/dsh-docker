    ...
    h('section', { className: css.section, 'aria-labelledby': 'platform-proxy-master-title' },
      h('div', { className: `${css.proxySettingRow} ${css.proxyMasterRow}` },
        ...
        h('label', { className: css.toggle, 'aria-label': t('proxyMaster'), ...toggleScrollHandlers },
          h('input', { type: 'checkbox', checked: configuration.enabled === true, disabled: busy || componentUnavailable, onChange: event => preserveToggleScroll(event.currentTarget, () => { void saveImmediate(next => { next.enabled = event.target.checked }) }) }),
          h('span', { 'aria-hidden': 'true' })))),
