      ...
      h('div', { className: `${css.proxySettingRow} ${css.proxyMasterRow}` },
        h('div', null,
          h('div', { className: css.proxyTitleLine },
            ...
            h('span', { className: css.proxyComponentState, 'data-state': configuration.componentReady ? 'ready' : 'unavailable', role: 'status' }, t(configuration.componentReady ? 'proxyReadyShort' : 'proxyUnavailableShort'))),
...
function apply(ctx) {
  ...
  ctx.effect(() => ctx.locale.register('settings.dshPlatformManagement', {
    zh: {
      ...
      proxyTitle: '代理设置', proxyDetail: '配置由 DSH Docker 使用的外部 HTTP 或 SOCKS5 代理。', proxyLoading: '正在加载代理配置…', proxyAdvanced: '高级', proxyUnsaved: '未保存', proxySavedShort: '已保存', proxyReadyShort: '已就绪', proxyUnavailableShort: '不可用', proxyProtocol: '代理协议', proxyHost: '主机', proxyPort: '端口', proxyUsername: '用户名', proxyPassword: '密码', proxyPasswordPlaceholder: '留空则保持当前密码', proxyRemoteDns: '通过代理解析域名', proxyRemoteDnsDetail: '仅适用于 SOCKS5；关闭时由容器本地解析。', proxyClearPassword: '清除密码', proxyTransportWarning: '当前页面未使用 HTTPS。代理凭据会受到传输链路保护能力的限制。', proxyComponentReady: '出站代理组件已就绪。配置只影响新建立的连接。', proxyComponentUnavailable: '出站代理组件当前不可用；可以保存配置，但连接测试可能失败。',
