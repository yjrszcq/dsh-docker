...
test('standalone console keeps localized feature parity on the shared Management API', async () => {
  ...
  assert.match(script, /systemPluginApplyingAction = \{ pluginId: id, action \}[\s\S]*waitForManagementTask/)
  ...
  assert.match(script, /const applyingAction = isActive[\s\S]*systemPluginApplyingAction\?\.pluginId === plugin\.id/)
