...
test('management serves only the fixed persistent Platform Management assets', async () => {
  ...
  try {
    ...
    const legacyRedirect = await rawRequest(socketPath, '/_dsh_platform/ui/app.js')
    assert.equal(legacyRedirect.status, 308)
    assert.equal(legacyRedirect.headers.location, '/_dsh_platform/console/app.js')
...
test('standalone console keeps localized feature parity on the shared Management API', async () => {
  ...
  assert.match(pluginSource, /href: '\/_dsh_platform\/auth\/\?next=\/_dsh_platform\/console\/'/)
