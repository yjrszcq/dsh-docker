...
test('standalone console keeps localized feature parity on the shared Management API', async () => {
  ...
  assert.match(style, /\.resource-panel \.list-pagination \{[^}]*position: sticky;[^}]*bottom: 0;/)
  ...
  assert.match(style, /\.log-list \{[\s\S]*height: min\(320px, 42dvh\);[\s\S]*min-height: 140px;[\s\S]*max-height: min\(720px, 70dvh\);[\s\S]*resize: none;/)
  assert.match(script, /makeLogListVerticallyResizable\(elements\['log-list'\]\)/)
  assert.match(script, /onResizeEdge = event => element\.getBoundingClientRect\(\)\.bottom - event\.clientY <= 10/)
  assert.match(script, /element\.style\.height = `\$\{String\(height\)\}px`/)
  assert.match(script, /element\.scrollIntoView\(\{ block: 'nearest', inline: 'nearest' \}\)/)
  assert.match(style, /@media \(max-width: 640px\)[\s\S]*\.log-list \{ height: min\(260px, 36dvh\); \}/)
