...
test('management exposes one reconnectable and cancellable outbound proxy test task', async () => {
  ...
  try {
    ...
    for (let attempt = 0; attempt < ASYNC_POLL_ATTEMPTS; attempt += 1) {
      audit = await logs.query({ sources: ['audit'] })
      if (audit.some(entry => entry.message === 'proxy.test.cancelled' && entry.taskId === taskId)) break
...
test('standalone Management disables a startup-faulting Bundle while DSH is already down', async () => {
  ...
  try {
    ...
    assert.equal(state.strategy, 'next-start')
...
test('standalone file task queue stays below file actions and scrolls within a fixed height', async () => {
  ...
  assert.match(style, /\.proxy-provider-list \{[^}]*max-height: 320px;[^}]*overflow-y: auto;/)
  assert.match(html, /id="proxy-advanced"[\s\S]*id="proxy-remote-dns-row"[^>]*hidden[\s\S]*id="proxy-remote-dns"/)
  assert.doesNotMatch(style, /\.proxy-check-row/)
  ...
  assert.match(script, /if \(tab === 'proxy'\) \{[\s\S]*if \(proxyLoaded\) void refreshProxyProviders\(\)[\s\S]*else void loadProxy\(\)/)
  assert.match(script, /function refreshVisibleProxyProviders\(\)[\s\S]*proxyLoaded[\s\S]*panel-proxy[\s\S]*refreshProxyProviders\(\)/)
  assert.match(script, /window\.addEventListener\('focus', refreshVisibleProxyProviders\)/)
  ...
  assert.doesNotMatch(html, /id="proxy-test-close"/)
  assert.match(html, /<footer><p id="proxy-test-result"[\s\S]*<button id="proxy-test-cancel"/)
  assert.match(html, /class="proxy-setting-row[^"]*"[\s\S]*id="proxy-enabled"/)
  ...
  assert.match(html, /<h3 data-i18n="proxyRules"[\s\S]*<h3 data-i18n="proxyAllProxy"/)
  assert.doesNotMatch(html, /<span data-i18n="proxyDirectRules"/)
  assert.match(style, /\.proxy-test-stage > span:nth-child\(2\) \{[^}]*display: grid;[^}]*gap: 1px;/)
  assert.match(style, /\.proxy-test-dialog footer \{[^}]*display: grid;[^}]*grid-template-columns: minmax\(0, 1fr\) auto;/)
  assert.match(style, /\.proxy-test-dialog footer > \.operation-result \{[^}]*margin: 0;/)
  assert.doesNotMatch(style, /@media \(max-width: 640px\)[\s\S]*\.proxy-test-dialog \{[^}]*height: 100dvh;/)
  assert.match(style, /\.proxy-scope-dialog \{[^}]*height: min\(760px, calc\(100dvh - 32px\)\)/)
  assert.match(style, /\.proxy-scope-catalog \{[^}]*min-height: 0;[^}]*overflow: auto;/)
  assert.match(script, /detail\.className = 'proxy-scope-description'[\s\S]*refreshProxyDescriptions\(\)/)
  assert.match(script, /function pendingProxyTestStages\(proxy = \{\}\)[\s\S]*stage !== 'target-dns'[\s\S]*proxy\.protocol === 'socks5'[\s\S]*status: 'pending'/)
  assert.match(script, /async function startProxyTest\(\)[\s\S]*status: 'starting'[\s\S]*pendingProxyTestStages\(\{[\s\S]*protocol:[\s\S]*remoteDns:[\s\S]*showModal\(\)/)
  assert.match(script, /function failedProxyTest\(error,[\s\S]*pendingProxyTestStages\(\)/)
  assert.match(script, /const expandedProxyTestStages = new Set\(\)/)
  assert.match(script, /item\.setAttribute\('role', 'button'\)[\s\S]*item\.setAttribute\('aria-expanded'[\s\S]*preserveScrollableAncestors\(item[\s\S]*detail\.hidden = !expanded/)
  assert.match(script, /statusLabel\.textContent = status/)
  assert.match(script, /proxy-test-cancel'[\s\S]*textContent = t\(running \? 'cancel' : 'close'\)/)
  ...
  assert.doesNotMatch(html, /id="proxy-no-proxy"|id="proxy-bypass"/)
  assert.doesNotMatch(style, /\.proxy-provider-description/)
...
test('standalone console keeps localized feature parity on the shared Management API', async () => {
  ...
  assert.match(html, /data-i18n="authMainPasswordTitle">Administrator account<\/h3><button id="auth-account-save"[^>]+data-i18n="authSaveApply"/)
  assert.match(html, /id="auth-account-save"[\s\S]*id="auth-username"[\s\S]*id="auth-current-password"/)
  assert.match(html, /data-i18n="authOriginTitle">Management origin<\/h3>[\s\S]*id="auth-origin-save"[^>]+data-i18n="authSaveApply"/)
  assert.match(html, /id="auth-origin-settings" hidden>[\s\S]*id="auth-origin" type="url"[\s\S]*id="auth-local-port" type="number" min="1" max="65535" step="1" inputmode="numeric" value="3081"[\s\S]*id="auth-origin-detect"/)
  ...
  assert.match(style, /\.auth-password-grid \{[^}]*align-items: start;/)
  ...
  assert.match(script, /function discardTotpChangeOnPageLeave\(\)[\s\S]*keepalive: true[\s\S]*window\.addEventListener\('pagehide', discardTotpChangeOnPageLeave\)/)
  ...
  assert.match(script, /window\.location\.replace\(returnPath\)/)
  assert.match(script, /window\.location\.assign\(new URL\(['"]\/['"], result\.loginOrigin\)\)/)
  assert.doesNotMatch(script, /window\.location\.(?:assign|replace)\(new URL\('\/auth\/management'/)
  assert.match(script, /const continuationOrigin = typeof result\.targetOrigin === 'string'/)
  assert.match(script, /isolatedEntry\?\.managementLocalOrigin/)
  ...
  assert.match(script, /authCurrentPasswordPlaceholder: '请输入当前主密码'/)
  assert.match(script, /authCurrentPasswordPlaceholder: 'Enter the current main password'/)
  assert.match(script, /authAdditionalTitle: '管理中心密码'/)
  assert.match(script, /authAdditionalTitle: 'Management console password'/)
  assert.match(script, /authNewPassword: '新密码'/)
  assert.match(script, /authConfirmPassword: '确认密码'/)
  assert.match(script, /authAdditionalPassword: '新密码'/)
  ...
  assert.match(script, /authLocalPort: '宿主机端口'/)
  assert.match(script, /const accessInput = elements\['auth-mode'\]\.value === 'isolated-local'[\s\S]*elements\['auth-local-port'\][\s\S]*elements\['auth-origin'\]/)
  assert.match(script, /!accessInput\.reportValidity\(\)/)
  assert.match(script, /changeManagementOrigin\(nextAccess\)/)
  assert.doesNotMatch(html, /auth-origin-current-password/)
  assert.doesNotMatch(script, /saveAuthenticationSettings/)
  assert.match(script, /elements\[confirmationId\]\.addEventListener\('input'[\s\S]*renderPasswordConfirmation/)
  assert.match(script, /auth-username'\]\.addEventListener\('input'/)
  assert.match(html, /id="auth-public-origin-field"[\s\S]*id="auth-origin"[\s\S]*id="auth-local-port-field" hidden[\s\S]*id="auth-local-port"[\s\S]*id="auth-origin-detect"/)
  ...
  assert.match(script, /function localizedProxyTestDetail\(stage, fallback\)/)
  assert.match(script, /detail\.textContent = localizedProxyTestDetail\(stage, status\)/)
  assert.match(script, /proxyStageDetailAddress: '已解析 \{count\} 个代理地址记录。'/)
  assert.match(script, /proxyStageDetailAddress: 'Resolved \{count\} proxy address record\(s\)\.'/)
  ...
  assert.doesNotMatch(script, /function saveProxyRules\(/)
  assert.match(script, /function saveProxyImmediate\(mutate\)[\s\S]*rules: 'saved'/)
  assert.match(script, /function proxyCandidate\([\s\S]*enabled: configuration\.enabled === true/)
  assert.match(script, /saveProxyImmediate\(configuration => \{ configuration\.enabled = event\.target\.checked \}\)/)
  assert.match(script, /const componentUnavailable = configuration\.componentReady === false/)
  assert.match(script, /proxy-enabled'\]\.disabled = componentUnavailable/)
  assert.match(script, /proxy-protocol', 'proxy-host', 'proxy-port', 'proxy-username', 'proxy-password'/)
  assert.match(script, /proxy-clear-password', 'proxy-advanced-toggle', 'proxy-direct-rules'/)
  assert.match(script, /proxy-system-rules', 'proxy-remote-dns', 'proxy-provider-search', 'proxy-scope-help'/)
  assert.match(script, /for \(const input of document\.querySelectorAll\('\[data-proxy-scope\]'\)\)[\s\S]*input\.disabled = componentUnavailable/)
  assert.match(script, /proxy-all-proxy'\]\.disabled = componentUnavailable/)
  assert.match(script, /proxy-test'\]\.disabled = componentUnavailable/)
  assert.match(script, /proxy-save'\]\.disabled = componentUnavailable/)
  assert.match(script, /proxy-password'\]\.disabled = proxyConfiguration\?\.componentReady === false/)
  assert.match(script, /draft\.clearPassword === 'true'[\s\S]*proxyConfiguration\?\.componentReady === false/)
  assert.match(script, /const componentUnavailable = proxyConfiguration\?\.componentReady === false[\s\S]*proxy-test'\]\.disabled = running \|\| componentUnavailable/)
  assert.match(script, /proxy-component-alert'\]\.hidden = !componentUnavailable/)
  assert.match(script, /proxy-component-alert'[\s\S]*proxy-component-dialog'\]\.showModal/)
  assert.doesNotMatch(script, /proxyReadyShort|proxyUnavailableShort/)
  assert.match(script, /function renderProxySaveState\(\{ saved = false \} = \{\}\)[\s\S]*proxyUnsaved[\s\S]*3000/)
  assert.match(script, /const displayName =[\s\S]*return query === '' \|\| displayName\.toLocaleLowerCase\(\)\.includes\(query\)/)
  assert.doesNotMatch(script, /\[provider\.displayName, provider\.id, provider\.type\]/)
  ...
  assert.match(script, /function resourceSearchValues\(key, value\) \{[\s\S]*if \(key === 'plugins' \|\| key === 'systemSkills'\) return \[value\.id\][\s\S]*if \(key === 'userSkills'\) return \[value\.name \?\? value\.entryName\][\s\S]*return \[value\.name\]/)
  ...
  assert.doesNotMatch(html, /class="page-heading"/)
  assert.doesNotMatch(script, /intro: 'DSH Docker/)
  ...
  assert.match(style, /body \{ position: fixed; inset: 0; width: 100%; height: 100dvh;[^}]*overflow: hidden; \}/)
  assert.match(style, /\.management-navigation \{[^}]*width: 100%;[^}]*min-width: 0;[^}]*max-width: 100%;[^}]*flex: none;[^}]*background: var\(--bg\);/)
  assert.doesNotMatch(style, /\.management-navigation \{[^}]*padding-top:/)
  assert.doesNotMatch(style, /\.page-heading/)
  assert.match(style, /\.tab-panel \{[^}]*min-height: 0;[^}]*flex: 1;[^}]*overflow-y: auto;[^}]*scrollbar-gutter: stable;/)
  assert.match(style, /\.resource-panel \.plugin-list, \.resource-panel \.user-plugin-list \{[^}]*max-height: min\(52dvh, 520px\);[^}]*flex: 0 1 auto;[^}]*overflow-y: auto;/)
  assert.match(style, /\.resource-panel \.plugin-list \{ min-height: 60px; \}/)
  assert.match(style, /\.resource-panel \.user-plugin-list \{ min-height: 96px; \}/)
  assert.match(style, /@media \(max-width: 640px\)[\s\S]*\.resource-panel \.plugin-list \{ min-height: 122px; \}[\s\S]*\.resource-panel \.user-plugin-list \{ min-height: 170px; \}/)
  assert.match(style, /\.resource-panel \.list-pagination \{[^}]*position: static;/)
  assert.doesNotMatch(style, /max-height: min\(calc\(56\.25vw - 27px\), 664px\)/)
  ...
  assert.match(script, /message\.textContent = display\(logMessageSummary\(entry\.message\)\)/)
  ...
  assert.equal(pluginSource.match(/href: managementConsoleHref/g)?.length, 2)
  assert.doesNotMatch(pluginSource, /href: '\/_dsh_platform\/auth\/management\/start/)
  ...
  assert.doesNotMatch(script, /scrollIntoView\(\{ block: 'nearest', inline: 'nearest' \}\)/)
  ...
  assert.match(script, /user-plugin-draft-actions'\]\.hidden = count === 0 && !restartRequired/)
  assert.match(script, /const dshUnavailable = status\?\.recoveryMode[\s\S]*applyUserPluginChangesOffline/)
  ...
  assert.match(style, /\.resource-panel \{ padding-bottom: 0; overflow-x: hidden; overflow-y: auto; overflow-anchor: none; \}/)
  assert.match(style, /\.resource-panel > section \{ height: 100%; min-height: 0; overflow: visible; \}/)
  assert.match(style, /\.resource-panel \.resource-section-heading,[\s\S]*\.resource-panel \.list-pagination,[\s\S]*\.resource-panel \.operation-result \{ flex: none; \}/)
  assert.match(style, /@media \(max-height: 600px\) \{\s*\.resource-panel > section \{ gap: 8px; padding: 12px 0; \}/)
  assert.match(style, /\.resource-panel \.plugin-list, \.resource-panel \.user-plugin-list \{[\s\S]*max-height: min\(52dvh, 520px\);[\s\S]*flex: 0 1 auto;[\s\S]*overflow-y: auto;[\s\S]*overflow-anchor: none;[\s\S]*scrollbar-gutter: stable/)
  assert.doesNotMatch(style, /\.resource-panel \.plugin-list, \.resource-panel \.user-plugin-list \{ max-height: min\(calc\(56\.25vw/)
  ...
  assert.match(script, /const stableScrollPositions = new WeakMap\(\)[\s\S]*if \(scrollRestoreGenerations\.has\(element\)\) return[\s\S]*scrollRestoreGenerations\.get\(current\) !== generation/)
  assert.match(script, /function preserveResourceRowPosition\(element, update\)[\s\S]*closest\('\.resource-panel'\)[\s\S]*current === panel\) break[\s\S]*current\.scrollTop \+= offset/)
  assert.match(script, /checkbox\.addEventListener\('change', event => preserveResourceRowPosition\(event\.currentTarget, \(\) => setSystemPluginDraft/)
  assert.match(script, /checkbox\.addEventListener\('change', event => preserveResourceRowPosition\(event\.currentTarget, \(\) => setUserPluginDraft/)
  assert.doesNotMatch(script, /scrollIntoView\(\{ block: 'nearest', inline: 'nearest' \}\)/)
  assert.match(script, /const tablist = activeTab\.parentElement[\s\S]*tablist\.scrollLeft = right - tablist\.clientWidth/)
  ...
  assert.match(style, /\.log-entry\[aria-expanded='false'\] \.log-message-row > pre \{[^}]*white-space: nowrap;/)
  ...
  assert.match(style, /button\.compact \{[^}]*border-radius: 6px;/)
  assert.match(style, /\.user-plugin-badge \{[^}]*border-radius: 4px;/)
  assert.match(style, /\.proxy-provider-heading > h2 \{[^}]*font-size: 16px;[^}]*line-height: 24px;/)
...
test('access clear-retry accepts only retry scope and credential selectors', async () => {
  ...
  assert.doesNotMatch(recoverError.message, /requires --image-baseline/)
  const tty = { isTTY: true }
  const calls = []
  const output = []
  assert.equal(await runCli({
    ...
    output: tty,
    ask: async prompt => {
      assert.equal(prompt, 'Clear Management console password login retry limits? y/[n]: ')
      return 'y'
    },
    write: line => output.push(line),
  }), 0)
