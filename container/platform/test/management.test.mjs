...
test('standalone console keeps localized feature parity on the shared Management API', async () => {
  ...
  assert.match(style, /\.resource-panel \.plugin-list, \.resource-panel \.user-plugin-list \{[^}]*min-height: 0;[^}]*max-height: none;[^}]*overflow-y: auto;/)
  ...
  assert.match(style, /\.resource-panel \{ padding-bottom: 0; overflow: hidden; overflow-anchor: none; \}/)
  assert.match(style, /\.resource-panel > section \{ height: 100%; min-height: 0; overflow: hidden; \}/)
  assert.match(style, /@media \(max-height: 600px\) \{\s*\.resource-panel > section \{ gap: 8px; padding: 12px 0; \}/)
  assert.match(style, /\.resource-panel \.plugin-list, \.resource-panel \.user-plugin-list \{[\s\S]*min-height: 0;[\s\S]*max-height: none;[\s\S]*flex: 0 1 auto;[\s\S]*overflow-y: auto;[\s\S]*overflow-anchor: none;[\s\S]*scrollbar-gutter: stable/)
