...
test('standalone console keeps localized feature parity on the shared Management API', async () => {
  ...
  assert.match(html, /id="auth-origin-settings" hidden>[\s\S]*id="auth-origin"[\s\S]*id="auth-origin-detect"/)
  ...
  assert.match(script, /auth-origin-settings'\]\.hidden = selected === 'compat'/)
  assert.match(html, /id="auth-origin-label"[\s\S]*id="auth-origin"/)
  assert.match(script, /authLocalPort: '宿主机端口'/)
  assert.match(script, /originInput\.type = local \? 'number' : 'url'/)
  assert.match(script, /originInput\.min = '1'[\s\S]*originInput\.max = '65535'/)
  assert.match(script, /auth-origin'\]\.reportValidity\(\)/)
