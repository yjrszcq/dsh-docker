...
test('standalone console keeps localized feature parity on the shared Management API', async () => {
  ...
  assert.match(style, /\.management-navigation \{[^}]*flex: none;/)
  ...
  assert.match(script, /stateKey = systemPluginSubmitting && action !== undefined[\s\S]*statusEnabling[\s\S]*statusDisabling/)
