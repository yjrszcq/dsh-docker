...
test('standalone file task queue stays below file actions and scrolls within a fixed height', async () => {
  ...
  assert.match(html, /id="proxy-rules-title"[\s\S]*id="proxy-rules-save"/)
...
test('standalone console keeps localized feature parity on the shared Management API', async () => {
  ...
  assert.match(script, /function saveProxyConfiguration\(\)[\s\S]*proxyCandidate\(\{ rules: 'saved' \}\)/)
  assert.match(script, /function saveProxyRules\(\)[\s\S]*proxyCandidate\(\{ connection: 'saved' \}\)/)
