...
test('standalone console keeps localized feature parity on the shared Management API', async () => {
  ...
  assert.match(statusLoader, /next = await api\('status'\)[\s\S]*setConnection\('online'\)[\s\S]*void loadInventories\(\)/)
  assert.doesNotMatch(statusLoader, /await loadInventories\(\)/)
  ...
  assert.match(script, /function runSkillTask\(path, body, operationKey\)[\s\S]*waitForManagementTask\(task\.taskId, operationKey\)[\s\S]*refreshInventories\(\)/)
