...
test('standalone file task queue stays below file actions and scrolls within a fixed height', async () => {
  ...
  assert.match(style, /\.proxy-check-row\[hidden\] \{ display: none !important; \}/)
  ...
  assert.match(html, /id="proxy-advanced-toggle"[^>]*aria-controls="proxy-advanced"[\s\S]*id="proxy-advanced"[\s\S]*id="proxy-system-rules"[\s\S]*id="proxy-direct-rules"[\s\S]*id="proxy-all-proxy"/)
