...
test('standalone console keeps localized feature parity on the shared Management API', async () => {
  ...
  assert.match(html, /id="language-switch"[^>]*data-i18n-aria-label="switchLanguage"/)
  ...
  assert.match(script, /elements\['language-switch'\]\.textContent = locale === 'zh' \? 'EN' : '中文'/)
  assert.match(script, /writeStorage\(LANGUAGE_KEY, locale === 'zh' \? 'en' : 'zh'\)[\s\S]*window\.location\.reload\(\)/)
  ...
  assert.match(style, /\.file-pathbar \.file-nav-button \{[^}]*width: 34px[^}]*height: 34px[^}]*font-size: 20px/)
  ...
  assert.match(style, /\.log-chevron::before \{[^}]*transform: rotate\(45deg\)[^}]*transform-origin: 50% 50%/)
  assert.match(style, /\.log-chevron \{[^}]*top: 2px/)
  assert.doesNotMatch(style, /\.log-chevron(?:::before)? \{[^}]*transition:/)
  assert.match(style, /\.log-entry\[aria-expanded='true'\] \.log-chevron::before \{[^}]*transform: rotate\(225deg\)/)
