...
test('standalone file task queue stays below file actions and scrolls within a fixed height', async () => {
  ...
  assert.match(style, /\.proxy-scope-description, \.proxy-provider-description \{[^}]*-webkit-line-clamp: 1;/)
  assert.match(script, /function refreshProxyDescriptions\(\)[\s\S]*preserveScrollableAncestors/)
  assert.match(script, /function renderProxyProviders\(\)[\s\S]*detail\.className = 'proxy-provider-description'/)
