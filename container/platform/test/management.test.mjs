...
test('standalone console keeps localized feature parity on the shared Management API', async () => {
  ...
  assert.match(html, /id="progress-steps" class="progress-steps"/)
  assert.match(html, /id="progress-metrics" class="progress-metrics" hidden/)
  assert.match(html, /id="progress-stage-log" class="progress-stage-log" hidden/)
  assert.match(html, /id="progress-log-auto-scroll" type="checkbox" checked/)
  assert.match(script, /function renderProgressSteps\(update\)/)
  assert.match(script, /function progressMetrics\(update\)/)
  assert.match(script, /taskId: String\(taskId\), limit: '1000'/)
