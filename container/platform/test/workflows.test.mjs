...
test('production publication owns signing, advances an append-only channel, and releases only Environment versions', async () => {
  ...
  assert.match(workflow, /uses: \.\/\.github\/workflows\/docker\.yaml/)
...
test('Docker publication consumes one signed channel target and mirrors only standard images to GHCR', async () => {
  ...
  assert.match(workflow, /DSH_RECOVERY_ROOT_PUBLIC_KEY:[\s\S]*required: false/)
  ...
  assert.match(workflow, /PLATFORM_REVISION=\$\{\{ inputs\.source_commit \}\}/)
  ...
  assert.doesNotMatch(workflow, /gh release|DSH_RELEASE_PRIVATE_KEY|prepare-release\.mjs/)
})
