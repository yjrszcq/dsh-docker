...
test('Docker publication has no signing key or metadata release authority', async () => {
  ...
  assert.match(workflow, /platform-\$\(jq -er \.targetSequence/)
  assert.match(workflow, /PLATFORM_REVISION=\$\{\{ github\.sha \}\}/)
  assert.match(workflow, /npm ci --omit=dev --ignore-scripts --prefix container\/control-plane\/services\/management/)
  assert.doesNotMatch(workflow, /inputs\.version|DSH_RELEASE_PRIVATE_KEY|prepare-release\.mjs/)
})
