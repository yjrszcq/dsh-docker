...
test('authorizes exactly one claim for each supervised DSH launch', async () => {
  ...
  assert.deepEqual(broker.readiness(), {
    generation: first.sessionId, publicReady: false, ready: false, readyUrl: null, verified: false,
  })
  assert.deepEqual(broker.ready(first.sessionId), { ready: true })
  assert.deepEqual(broker.readiness(), {
    generation: first.sessionId, publicReady: false, ready: true, readyUrl: null, verified: false,
  })
  broker.verify()
  assert.equal(broker.readiness().publicReady, false)
  broker.publish()
  assert.equal(broker.readiness().publicReady, true)
  ...
  assert.deepEqual(reports.map(value => value.message), [
    'dsh.launch.authorized', 'dsh.launch.claimed', 'dsh.launch.ready', 'dsh.launch.verified',
    'platform.public-ready', 'dsh.launch.released',
  ])
...
test('invalidates an old launch without allowing its cleanup to release the replacement', () => {
  ...
  const oldSession = broker.claim(oldLaunch.environment.DSH_PLATFORM_LAUNCH_TOKEN)
  broker.ready(oldSession.sessionId)
  broker.verify()
  broker.publish()
  assert.equal(broker.readiness().publicReady, true)
...
test('serves launch ownership and readiness over a private Unix socket', async t => {
  ...
  assert.deepEqual(await call(socketPath, '/v1/runtime/readiness', {}), {
    status: 200,
    body: {
      generation: claim.body.sessionId, publicReady: false, ready: false, readyUrl: null, verified: false,
    },
  })
  ...
  assert.deepEqual(await call(socketPath, '/v1/runtime/readiness', {}), {
    status: 200,
    body: {
      generation: claim.body.sessionId, publicReady: false, ready: true, readyUrl, verified: false,
    },
  })
  broker.verify()
  broker.publish()
  assert.equal((await call(socketPath, '/v1/runtime/readiness', {})).body.publicReady, true)
