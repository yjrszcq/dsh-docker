...
test('authorizes exactly one claim for each supervised DSH launch', async () => {
  ...
  assert.deepEqual(broker.readiness(), { generation: first.sessionId, ready: false, readyUrl: null })
  assert.deepEqual(broker.ready(first.sessionId), { ready: true })
  assert.deepEqual(broker.readiness(), { generation: first.sessionId, ready: true, readyUrl: null })
...
test('serves launch ownership and readiness over a private Unix socket', async t => {
  ...
  assert.deepEqual(await call(socketPath, '/v1/runtime/readiness', {}), {
    status: 200,
    body: { generation: claim.body.sessionId, ready: false, readyUrl: null },
  })
  ...
  assert.deepEqual(await call(socketPath, '/v1/runtime/readiness', {}), {
    status: 200,
    body: { generation: claim.body.sessionId, ready: true, readyUrl },
  })
