...
test('builds a self-contained Bootstrap seed and preserves npm bin links', async () => {
  ...
  const bootstrapVersion = (await readFile(new URL('../bootstrap/VERSION', import.meta.url), 'utf8')).trim()
  const bootstrapRoot = join(output, 'bootstrap', bootstrapVersion)
  const contracts = await import(pathToFileURL(join(bootstrapRoot, 'platform/lib/contracts.mjs')).href)
  ...
  assert.equal(inventory.bootstrap.version, bootstrapVersion)
