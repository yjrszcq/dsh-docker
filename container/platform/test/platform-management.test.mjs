...
test('Platform Management follows DSH settings tokens and responsive layout', async () => {
  ...
  assert.match(style, /\.logList \{[\s\S]*height: min\(280px, 42dvh\);[\s\S]*min-height: 140px;[\s\S]*max-height: min\(720px, 70dvh\);[\s\S]*resize: none;/)
  assert.match(source, /onResizeEdge = event => list\.getBoundingClientRect\(\)\.bottom - event\.clientY <= 10/)
  assert.match(source, /list\.style\.height = `\$\{String\(height\)\}px`/)
  assert.match(source, /list\.scrollIntoView\(\{ block: 'nearest', inline: 'nearest' \}\)/)
  ...
  assert.match(style, /@media \(max-width: 640px\)[\s\S]*\.logList \{ height: min\(260px, 36dvh\); \}/)
