...
test('Platform Management is embedded in the official settings.section slot', async () => {
  ...
  const refreshStatus = managementSource.slice(managementSource.indexOf('const refresh = useCallback'), managementSource.indexOf('const refreshInventories = useCallback'))
  ...
  const refreshInventories = managementSource.slice(managementSource.indexOf('const refreshInventories = useCallback'), managementSource.indexOf('const act = useCallback'))
  assert.match(refreshInventories, /Promise\.all\(\[request\('bundled-plugins'\), request\('system-skills'\)\]\)/)
  assert.match(source, /void refreshInventories\(\)[\s\S]*await refreshAndConnect\(\)/)
