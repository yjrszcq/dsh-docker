...
test('Platform Management is embedded in the official settings.section slot', async () => {
  ...
  assert.match(systemPluginManager, /const applyingAction = applyingDraft\.get\(plugin\.id\) \?\? \(isActive \? operation\.action : undefined\)/)
...
test('Platform Management follows DSH settings tokens and responsive layout', async () => {
  ...
  assert.match(source, /SystemPluginManager\(\{ plugins, draft, applyingDraft,[\s\S]*const applyingAction = applyingDraft\.get\(plugin\.id\)[\s\S]*statusEnabling[\s\S]*statusDisabling/)
