...
test('Platform Management is embedded in the official settings.section slot', async () => {
  ...
  assert.match(source, /managementHref = typeof context\?\.managementOrigin === 'string'/)
  assert.match(source, /href: managementHref[\s\S]*target: '_blank'[\s\S]*t\('openPlatformManagement'\)/)
