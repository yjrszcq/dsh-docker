...
test('Platform Management is embedded in the official settings.section slot', async () => {
  ...
  assert.match(source, /const saveConnection = async[\s\S]*candidate\(\{ rules: 'saved' \}\)/)
  assert.match(source, /const saveRules = async[\s\S]*connectionSource: proxyConnectionDraft\(configuration\)/)
  ...
  assert.match(source, /saveRules[\s\S]*includeSecret: false/)
  ...
  assert.match(source, /className: css\.proxySettingRow[\s\S]*className: css\.toggle[\s\S]*configuration\.environment\.allProxy/)
  ...
  assert.match(source, /configuration\.environment\.allProxy[\s\S]*saveImmediate\(next => \{ next\.environment\.allProxy/)
