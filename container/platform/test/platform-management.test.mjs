...
test('Platform Management declares a DSH web client and a platform-namespaced overlay row', async () => {
  ...
  assert.equal(packageJson.dshDocker.description.zh, '管理 DSH Docker 更新、运行维护、系统资源与登录会话。')
...
test('Platform Management is embedded in the official settings.section slot', async () => {
  ...
  const authentication = source.slice(source.indexOf('function AuthenticationSettings('), source.indexOf('\nfunction UpdateDashboard('))
  assert.match(authentication, /`\$\{managementConsoleHref\}#auth-settings`/)
  assert.doesNotMatch(source, /\/_dsh_platform\/auth\/management\/start/)
  assert.match(source, /setManagementConsoleHref\(value\.managementConsoleHref/)
  assert.match(source, /addEventListener\('focus', refreshManagementConsoleHref\)/)
  ...
  assert.match(source, /filteredPlugins = plugins\.filter\(plugin => matchesResourceSearch\(query, \[plugin\.id\]\)\)/)
  assert.match(source, /filteredSkills = skills\.filter\(skill => matchesResourceSearch\(query, \[skill\.id\]\)\)/)
  assert.doesNotMatch(source, /matchesResourceSearch\(query, \[(?:plugin|skill)\.id, (?:plugin|skill)\.description/)
  ...
  assert.match(source, /updatesTab: '更新管理', maintenanceTab: '运行维护', pluginsTab: '系统插件', skillsTab: '系统技能', authenticationTab: '认证设置', proxyTab: '代理设置'/)
  assert.match(source, /updatesTab: 'Updates', maintenanceTab: 'Maintenance', pluginsTab: 'System plugins', skillsTab: 'System skills', authenticationTab: 'Authentication', proxyTab: 'Proxy'/)
  assert.match(source, /\['maintenance', 'plugins', 'skills', 'proxy', 'updates', 'authentication'\]/)
  assert.match(source, /authenticationRequest\('session-context'\)/)
  assert.match(source, /authenticationRequest\('browser-logout', \{ method: 'POST', body: \{ scope \} \}\)/)
  assert.match(authentication, /scope === 'all'[\s\S]*closePlatformStateEvents\(\)[\s\S]*authenticationRequest\('browser-logout'/)
  assert.match(authentication, /catch \(nextError\) \{[\s\S]*openPlatformStateEvents\(\)/)
  assert.match(authentication, /window\.setTimeout\(\(\) => setMessage\(''\), 3_000\)/)
  assert.match(authentication, /window\.clearTimeout\(timer\)/)
  assert.match(source, /dsh-platform access reset/)
  assert.match(source, /const \[activeTab, setActiveTab\] = useState\('maintenance'\)/)
  assert.match(source, /\['maintenance', 'plugins', 'skills', 'proxy', 'updates', 'authentication'\]\.map/)
  ...
  assert.match(style, /\.primaryButton,[\s\S]*?\.smallButton \{[^}]*border-radius: 18px;/)
  assert.match(style, /\.smallButton \{[^}]*border-radius: 14px;/)
  assert.match(style, /\.toggle > span \{[^}]*border-radius: 10px;/)
  assert.match(style, /\.statusBadge \{[^}]*display: inline-flex;[^}]*min-height: 18px;[^}]*border-radius: 999px;/)
  assert.match(style, /\.managedBadge \{[^}]*border-radius: 12px;/)
  assert.match(style, /\.autoScrollButton,[\s\S]*?\.clearLogsButton \{[^}]*border-radius: 999px;/)
  assert.match(style, /\.logAutoScroll > span \{[^}]*border-radius: 8px;/)
  ...
  assert.match(source, /const toggleProxy = event =>/)
  assert.match(source, /toggleProxy[\s\S]*saveImmediate\(next => \{ next\.enabled = event\.target\.checked \}\)/)
  ...
  assert.match(source, /const componentUnavailable = configuration\.componentReady === false/)
  assert.match(source, /checked: configuration\.enabled === true, disabled: busy \|\| componentUnavailable/)
  assert.match(source, /disabled: busy \|\| taskRunning \|\| componentUnavailable/)
  assert.match(source, /value: connection\.protocol,[\s\S]*disabled: componentUnavailable,[\s\S]*items: \[\{ id: 'http', label: 'HTTP' \}, \{ id: 'socks5', label: 'SOCKS5' \}\]/)
  assert.match(source, /value: connection\.host, disabled: componentUnavailable/)
  assert.match(source, /value: connection\.username, disabled: componentUnavailable/)
  assert.match(source, /value: password, disabled: clearPassword \|\| componentUnavailable/)
  assert.match(source, /value: directRulesText, disabled: componentUnavailable/)
  assert.match(source, /value: providerQuery, disabled: componentUnavailable/)
  assert.match(source, /checked: configuration\.scopes\[id\], disabled: busy \|\| componentUnavailable/)
  assert.match(source, /disabled: provider\.routingCapability === 'forced-direct' \|\| componentUnavailable/)
  ...
  assert.match(style, /\.proxyComponentAlert path \{ stroke: var\(--dsw-alias-label-primary-foreground\)/)
  ...
  const proxySettings = source.slice(source.indexOf('function ProxySettings('), source.indexOf('\nfunction PlatformManagement('))
  ...
  assert.equal(source.match(/href: managementConsoleHref/g)?.length, 2)
  assert.doesNotMatch(source, /href: '\/_dsh_platform\/auth\/management\/start/)
  ...
  assert.match(source, /const generation = connection\.generation \?\? connection\.hostDescription/)
  assert.match(source, /generation\.subscribe\(connectionChanged\)/)
  ...
  assert.match(source, /closeReminder: '关闭更新提醒'/)
  assert.match(source, /closeReminder: 'Close update reminder'/)
  assert.match(source, /previousFocus\.current\?\.isConnected/)
  assert.match(source, /'data-notice-action': 'snooze', 'aria-label': t\('closeReminder'\)/)
...
test('Platform Management follows DSH settings tokens and responsive layout', async () => {
  ...
  assert.match(source, /function preserveResourceRowPosition\(element, update\)[\s\S]*current === panel\) break[\s\S]*querySelectorAll\('article'\)[\s\S]*current\.scrollTop \+= offset/)
  assert.match(source, /onChange: event => preserveResourceRowPosition\(event\.currentTarget, \(\) => onAction\(plugin, event\.target\.checked \? 'enable' : 'disable'\)\)/)
  assert.match(source, /onChange: event => preserveResourceRowPosition\(event\.currentTarget, \(\) => onAction\(skill, event\.target\.checked \? 'enable' : 'disable'\)\)/)
  ...
  assert.match(style, /\.pluginSection \{[^}]*height: 100%;[^}]*min-height: 0;[^}]*overflow: visible;/)
  assert.match(style, /\.pluginSection > \.resourceSectionHeading,[\s\S]*\.pluginSection > \.listPagination,[\s\S]*\.pluginSection > \.error \{ flex: none; \}/)
  assert.match(style, /\.pluginList \{[^}]*min-height: 68px;[^}]*max-height:[^}]*flex: 0 1 auto;[^}]*overflow-y: auto;/)
  assert.match(style, /\.pluginSection \.listPagination \{ position: static; \}/)
  assert.match(style, /@media \(max-width: 640px\)[\s\S]*\.pluginList \{ min-height: 122px; \}/)
  ...
  assert.match(source, /function localizedProxyTestDetail\(stage, t, fallback\)/)
  assert.match(source, /localizedProxyTestDetail\(stage, t, status\)/)
  assert.match(source, /proxyStageDetailAddress: '已解析 \{count\} 个代理地址记录。'/)
  assert.match(source, /proxyStageDetailAddress: 'Resolved \{count\} proxy address record\(s\)\.'/)
  assert.match(source, /function pendingProxyTestStages\(proxy = \{\}\)[\s\S]*stage !== 'target-dns'[\s\S]*proxy\.protocol === 'socks5'[\s\S]*status: 'pending'/)
  assert.match(source, /const startTest = async \(\) => \{[\s\S]*status: 'starting'[\s\S]*pendingProxyTestStages\(connection\)[\s\S]*showModal\(\)/)
  ...
  assert.doesNotMatch(style, /@media \(max-width: 640px\)[\s\S]*\.proxyTestDialog \{[^}]*height: 100dvh;/)
  assert.match(style, /\.proxyScopeDialog \{[^}]*height: min\(720px, calc\(100dvh - 40px\)\)/)
  assert.match(style, /\.proxyScopeCatalog \{[^}]*min-height: 0;[^}]*overflow: auto;/)
  assert.match(source, /h\(ExpandableProxyDescription, \{ text: localize\(entry\.detail\), identity: entry\.id \}\)/)
  ...
  assert.match(style, /body\[data-dsh-platform-select-menu='open'\] > \[role='menu'\][\s\S]*?max-height: min\(320px, 60dvh\);[\s\S]*?overflow: hidden;/)
  assert.match(style, /body\[data-dsh-platform-select-menu='open'\] > \[role='menu'\] > \[role='presentation'\][\s\S]*?overflow-y: auto;/)
  assert.match(source, /dataset\.dshPlatformSelectMenu = 'open'/)
  assert.match(source, /proxyEndpointRequired/)
  assert.match(source, /addEventListener\('sources'/)
  assert.match(style, /\.proxyFormGrid input:focus-visible,[\s\S]*?border-color: var\(--dsw-alias-label-primary\);[\s\S]*?outline: none;/)
