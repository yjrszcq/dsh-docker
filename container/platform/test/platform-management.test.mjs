...
test('Platform Management is embedded in the official settings.section slot', async () => {
  ...
  assert.match(authentication, /href: '\/_dsh_platform\/auth\/management\/start\?tab=auth-settings'/)
  ...
  assert.equal(source.match(/href: '\/_dsh_platform\/auth\/management\/start'/g)?.length, 1)
  assert.equal(source.match(/href: '\/_dsh_platform\/auth\/management\/start\?tab=auth-settings'/g)?.length, 1)
