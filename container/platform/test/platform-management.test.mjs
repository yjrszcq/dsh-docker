...
test('Platform Management is embedded in the official settings.section slot', async () => {
  ...
  assert.match(source, /href: '\/_dsh_platform\/ui\/'/)
  assert.match(source, /fetch\(`/)
  assert.match(source, /new EventSource/)
  assert.match(source, /const value = await refresh\(\)[\s\S]*void checkUpdates\('page-open'\)/)
  ...
  for (const route of ['status', 'check', 'update', 'channel', 'automatic-check', 'holds\\/retry', 'rollback', 'return-stable', 'restart-dsh', 'bundled-plugins', 'bundled-plugins\\/toggle', 'bundled-plugins\\/discard']) {
  ...
  assert.match(source, /systemPluginsDetail: '启用或禁用当前已安装的 DSH Docker 系统插件。'/)
  assert.match(source, /plugins\.some\(plugin => plugin\.pendingRestart\)/)
  assert.match(source, /visiblePlugins = plugins\.filter\(plugin => plugin\.installed\)/)
  assert.match(source, /href: '\/_dsh_platform\/ui\/'/)
  ...
  assert.match(source, /sessionStorage\.setItem\(PLUGIN_DRAFT_KEY, '1'\)[\s\S]*bundled-plugins\/toggle/)
  ...
  assert.doesNotMatch(systemPluginManager, /onAction\(plugin\.id, '(?:install|uninstall)'\)/)
