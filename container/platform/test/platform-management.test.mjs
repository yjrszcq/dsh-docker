...
test('Platform Management is embedded in the official settings.section slot', async () => {
  ...
  assert.match(systemPluginManager, /const applyingAction = isActive[\s\S]*applying\?\.pluginId === plugin\.id/)
...
test('Platform Management follows DSH settings tokens and responsive layout', async () => {
  ...
  assert.match(source, /SystemPluginManager\(\{ plugins, draft, applying,[\s\S]*const applyingAction = isActive[\s\S]*statusEnabling[\s\S]*statusDisabling/)
