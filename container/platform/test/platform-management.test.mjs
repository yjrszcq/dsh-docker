...
test('Platform Management is embedded in the official settings.section slot', async () => {
  ...
  assert.match(source, /pluginPendingRestart: '待应用'/)
  ...
  assert.match(systemPluginManager, /h\(ResourceStatusBadge[\s\S]*pending: isActive \|\| action !== undefined \|\| plugin\.pendingRestart/)
...
test('Platform Management follows DSH settings tokens and responsive layout', async () => {
  ...
  assert.match(source, /SystemPluginManager\(\{ plugins, draft, applying,[\s\S]*stateKey = applying && action !== undefined[\s\S]*statusEnabling[\s\S]*statusDisabling/)
  assert.match(style, /\.root \{[^}]*height: min\(720px, calc\(100dvh - 152px\)\);[^}]*overflow: hidden;/)
  assert.match(style, /\.platformHeader \{[^}]*flex: none;[^}]*background: var\(--dsw-alias-bg-layer-2\);/)
