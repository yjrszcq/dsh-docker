...
function usage() {
  return 'usage: dsh-platform status|check|update [--wait]|rollback|logs [--source NAME] [--since ISO] [--limit N]|trust status|reset'
}

export function parseCli(argv) {
  const [command, ...rest] = argv
  if (command === 'status' || command === 'check' || command === 'rollback') {
