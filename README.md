...
The Access Manager persists the administrator account. DSH and the Management Console use separate sessions, with optional Management-console password and two-factor authentication. The default Compose setting keeps `DSH_SUDO_ENABLED=false`. Preserve both `/data/dsh` and `/data/platform` (especially `/data/platform/state`) when replacing the image. Removing the platform volume resets the administrator account and authentication state and can route an existing deployment into migration or recovery.
